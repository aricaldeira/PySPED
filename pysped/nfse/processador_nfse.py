...
class ProcessadorNFSe(object):
    ...
    def _configuracao_municipio(self, nfe):
    ...
    def _prepara_conexao(self, operacao):
        conexao = Conexao()
    ...
    def _limpa_namespace(self, operacao, xml):
    ...
    def _assina_xml_rps(self, nfe):
        self._configuracao_municipio(nfe)
        ...
        if str(nfe.infNFe.emit.enderEmit.cMun.valor) == '3550308':
            assinatura = assinatura_nfse_sp(nfe)
        ...
        self.certificado.verifica_assinatura_xml(conteudo)

        return conteudo

    def envia_rps(self, nfe):
        self._configuracao_municipio(nfe)
        conteudo = self._assina_xml_rps(nfe)
        ...
        conexao = self._prepara_conexao('envio_rps')
        ...
        xml = self._limpa_namespace('envio_rps', xml)
        ...
        if tag_sucesso:
            ...
            #conexao.erros.append(conexao.xml_resposta)
            #conexao.mensagem_erro = conexao.xml_resposta

        return conexao


def assinatura_nfse_sp(NFe):
